...
parser = argparse.ArgumentParser(description='Run inferences with different models, LAMBDA, and GPU Frequency values.')
parser.add_argument('--model', type=str, default='vit_b_16', help='Model name (default: vit_b_16)')
parser.add_argument('--lambda', dest='lambda_value', type=int, default=32, help='The lambda value to control the rate of request submission (default: 32)')
parser.add_argument('--gpu-freq', dest='gpu_frequency', type=int, default=200, help='GPU frequency setting (default: 200)')
parser.add_argument('--images-size', dest='images_size', type=int, default=100, help='Images Size (default: 100)')
args = parser.parse_args()

TORCHSERVE_URL = f'http://localhost:8080/predictions/{args.model}'
IMAGE_DIR = '/media/Extra/ImageNet-2012-Dataset/imagenet/val/'
MAX_IMAGES = args.images_size
LAMBDA = args.lambda_value
FREQ_DIR = f'../records/{args.model}/freq_{args.gpu_frequency}/'
...
async def collect_gpu_metrics(interval, stop_event):
    ...
    pd.DataFrame([x.split(', ') for x in metrics]).to_csv(os.path.join(FREQ_DIR, f'λ{LAMBDA}_gpu_metrics.csv'), header=["Index", "Timestamp", "Power Draw (W)", "GPU Clock (MHz)", "Memory Clock (MHz)", "Temperature (C)", "GPU Utilization (%)", "Memory Utilization (%)", "Total Memory (MiB)", "Used Memory (MiB)"], index=False)
...
async def main():
    ...
    gpu_metrics_df = pd.read_csv(os.path.join(FREQ_DIR, f'λ{LAMBDA}_gpu_metrics.csv'))
    gpu_metrics_summary = process_gpu_metrics(gpu_metrics_df)

    latency_df = pd.DataFrame(latency_data)
    latency_df.to_csv(os.path.join(FREQ_DIR, f'λ{LAMBDA}_latencies.csv'), index=False)
    ...
    average_metrics_file = os.path.join(FREQ_DIR, 'average_metrics.csv')
